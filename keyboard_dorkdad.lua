...
function UIBox.init(self, args)
	if not args.config.parent then
		G.ORPHANED_UIBOXES[self] = true
	end
...
function Controller.key_press_update(self, key, dt)
	...
	elseif G.STATE == G.STATES.SHOP then
		if
			key == "r"
			and not ((G.GAME.dollars - G.GAME.bankrupt_at) - G.GAME.current_round.reroll_cost < 0)
			and G.GAME.current_round.reroll_cost ~= 0
		then
...
function invert_selection()
	local unselected = filter(G.hand.cards, function(x)
		return -1 == indexOf(G.hand.highlighted, function(y)
			return y == x
		end)
	end)
...
function are_ranks_same(hand1, hand2)
	...
	for k, v in pairs(h1_rank_counts) do
		if not h2_rank_counts[k] then
			return false
		end
		if not (h2_rank_counts[k] == v) then
			return false
		end
	end
	-- this is a bad solution but this part is not computation intensive anyways
	for k, v in pairs(h2_rank_counts) do
		if not h1_rank_counts[k] then
			return false
		end
		if not (h1_rank_counts[k] == v) then
			return false
		end
	end
	return true
...
function possible_hands(cards, prop_selector)
	...
	table.sort(res, function(x, y)
		return ((#x == #y) and (hand_importance(x) > hand_importance(y))) or #x > #y
	end)
	return res
end
...
function select_hand(cards)
	G.hand:unhighlight_all()
	for k, v in pairs(cards) do
		if indexOf(G.hand.highlighted, function(x)
			return x == v
		end) == -1 then
